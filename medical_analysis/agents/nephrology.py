...
class NephrologyAgent(MedicalAgent):
    def __init__(self, model: str = "gemini-pro", use_gpu: bool = True):
        super().__init__(specialty="nephrology", model=model, use_gpu=use_gpu)

    def analyze(self, content: str, use_cache: bool = True):
        prompt = self.prompts.get("analysis", "").format(content=content[:10000])
        cot_prompt = (
            f"Let's think step by step as a nephrologist.\n"
            f"{prompt}\n"
            f"Step 1: Identify renal function and urine findings.\n"
            f"Step 2: Review electrolytes and fluid status.\n"
    ...
    def answer_query(self, query: str, context: dict):
        ...
        cot_prompt = (
            ...
            f"Step 3: Apply nephrology knowledge.\n"
