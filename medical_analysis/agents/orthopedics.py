...
class OrthopedicsAgent(MedicalAgent):
    def __init__(self, model: str = "gemini-pro", use_gpu: bool = True):
        super().__init__(specialty="orthopedics", model=model, use_gpu=use_gpu)

    def analyze(self, content: str, use_cache: bool = True):
        prompt = self.prompts.get("analysis", "").format(content=content[:10000])
        cot_prompt = (
            f"Let's think step by step as an orthopedist.\n"
            f"{prompt}\n"
            f"Step 1: Identify musculoskeletal symptoms and imaging findings.\n"
            f"Step 2: Review functional status and injury history.\n"
    ...
    def answer_query(self, query: str, context: dict):
        ...
        cot_prompt = (
            ...
            f"Step 3: Apply orthopedics knowledge.\n"
